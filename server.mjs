...
export async function createApp({dataDir,saveDir=null,port=47831,interval=5000}={}) {
  ...
  const assets=new Map(['/','/index.html','/app.js','/map.js','/style.css','/icon.svg','/reference.js','/reference-core.js','/data-client.js','/career.js','/studio.js','/field-library.js','/field-theme.css','/hunting-workspace.css','/map-geometry.js','/terrain-layer.js','/map-atlas.js','/maps.css'].map(url=>[url,readFileSync(path.join(appDir,'public',url==='/'?'index.html':url.slice(1)))]));
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      if(req.method==='POST'&&url.pathname==='/api/command') {
        ...
        if(body.op==='observer.scan'){await observer.scan(true);return json(200,{scanned:true,lastCycle:observer.lastCycle});}
        const result=store.mutate(observer.profile,body.requestId,body,()=>store.command(observer.profile,body));return json(200,result);
      }
  ...
  const close=async()=>{if(legacyRedirect)legacyRedirect.close();observer.stop();while(observer.busy)await new Promise(r=>setTimeout(r,25));await new Promise(r=>server.close(r));store.close();};
  return {server,store,observer,close,url:`http://127.0.0.1:${server.address().port}`};
}
