...
try{
 ...
 await page.goto(app.url+'/#settings',{waitUntil:'domcontentloaded'});
 ...
 proof.checks.push('Two unchanged 5-second state polls and one 10-second update-status poll kept Settings, phone, update panel, open details, and focus intact');
 ...
 await sleep(100);
 assert.equal(await page.evaluate(()=>window.__zoneMarker===document.querySelector('#fieldMap g[data-zone]')),true,'unrelated encounter must not rebuild the same zone marker');
