...
export class Observer {
  ...
  command(body){return this.store.command(this.profile,body,{setupContext:reserve=>({route:this.routeState(reserve).route,structures:this.publicPlaces(reserve)}),routePlanner:(reserve,route,options)=>this.routeState(reserve,route,options),canRenamePlace:id=>[...this.pinPlaces(),...this.equipmentPlaces(),...Object.values(this.reference.reserves).flatMap(r=>this.publicPlaces(r.id))].some(p=>p.canRename===true&&p.renameId===id)});}
