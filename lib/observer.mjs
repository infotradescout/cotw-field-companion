...
export class Observer {
  ...
  async scan(force=false) {
    ...
    try {
      ...
      for(const name of list) {
        try {
          const prior=this.source(name),s=await stat(path.join(this.root,name)),needsProjectionRefresh=name.startsWith('animal_population_')&&prior?.payload?.projectionVersion!==2;
  ...
  state(reserve) {
    ...
    const reserves=Object.values(this.reference.reserves).map(r=>({...r,poi:r.id===reserve?(r.poi||[]):[],available:!!this.source('animal_population_'+r.id)?.payload,status:statusByName['animal_population_'+r.id]?.status??'not_found',zoneCount:zones.filter(z=>z.reserve===r.id).length}));
    return {
      ...
      huntingPressure:huntingPressureView(population?.huntingPressure,{reserve,bounds:this.reference.reserves[reserve]?.bounds,savedAt:this.source('animal_population_'+reserve)?.mtime??null,sourceStatus:statusByName['animal_population_'+reserve]?.status??'unavailable'}),
