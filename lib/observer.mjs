...
import {observerSaveData,recordProfileProgress} from './save-data.mjs';
...
export class Observer extends SaveObserver{
 ...
 async scan(force=false){
  ...
  await super.scan(force);
  try{recordProfileProgress(this.store,this.profile,this.source('thp_player_profile_adf'),this.lastError?'error':this.store.sources(this.profile).find(s=>s.name==='thp_player_profile_adf')?.status);this.saveDataHistoryError=false;}
  catch{this.saveDataHistoryError=true;}
 }
