...
import {decodeSave} from './decoder.mjs';
...
const EXTRA=['found_need_zones_adf','hunting_log_adf','worlditemsdata_adf','health_component_manager_adf','reserveworlddata_adf'];
...
export class Observer {
    ...
    try {
      ...
      for(const name of list) {
        try {
          ...
          const decoded=decodeSave(read.b),payload=normalizeFile(name,decoded);
  ...
  state(reserve) {
    ...
    return {
      app:{name:'COTW Field Companion',version:'0.3.0',release:'Product repair candidate',startedAt:this.startedAt},profile,selectedReserve:reserve,reserves,settings,
      ...
      zones:selected,pins:s.journal(profile,'pins').filter(p=>p.reserve===reserve),equipment:(this.source('worlditemsdata_adf')?.payload??[]).filter(p=>p.reserve===reserve),
