...
export function sessionHarvestSummary(session,harvests){
 const start=Date.parse(session.startedAt),end=session.endedAt?Date.parse(session.endedAt):Infinity;
 if(!Number.isFinite(start)||Number.isNaN(end)||end<start)return null;
 const species=new Map();let total=0;
 for(const h of harvests){const time=h.timestamp*1000;if(!Number.isFinite(time)||time<start||time>end)continue;total++;species.set(h.species,(species.get(h.species)||0)+1);}
