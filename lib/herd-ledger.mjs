...
function describe(payload,reserve){
 ...
 for(const p of payload.populations){
  ...
  for(const [index,g]of p.groups.entries()){
   if(!Array.isArray(g?.animals)||!Array.isArray(g.paths)||g.paths.length>51||g.paths.some(v=>!uint(v))||!uint(g.area))throw Error('Invalid group assignment');
   ...
   const anchor=hash([g.area,g.paths]);
   groups.push({speciesHash:p.hash,index,epoch,area:g.area,paths:[...g.paths],members,signature:hash(members),anchor,animals:g.animals.length});
  }
 }
