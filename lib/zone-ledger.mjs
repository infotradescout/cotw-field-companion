...
export function zoneCommand(store,profile,body,{zones,route=[],sourceStatus='unavailable',now=new Date().toISOString()}={}){
 ...
 if(body.op==='zone.track'){
  ...
  if(!Number.isSafeInteger(body.expectedVersion)||body.expectedVersion!==previous.version)fail('Tracking changed. Refresh before choosing a zone.',409);
