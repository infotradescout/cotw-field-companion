import {createPhoneBridge,provisionPhoneDevice,phoneRelayOrigin} from './phone-bridge.mjs';

/** One explicitly enabled phone connection for the selected local save profile. */
export function createPhoneAccess({store,observer,relayUrl=null,enrollmentToken,runCommand,allowInsecureLoopback=false,provision=provisionPhoneDevice,bridgeFactory=createPhoneBridge}={}){
  const origin=relayUrl?phoneRelayOrigin(relayUrl,{allowInsecureLoopback}):null;
  const key='phone:connection:'+observer.profile;
  ...
  const makeBridge=(connection,ticket)=>{let candidate;candidate=bridgeFactory({relayUrl:connection.relayUrl,deviceToken:connection.deviceToken,readState:reserve=>observer.state(reserve),runCommand,allowInsecureLoopback,onStatus:event=>{if(ticket===generation&&bridge===candidate)status=event.status;}});return candidate;};
  ...
  return {
    ...
    async enable(body){
      ...
      try{
        const credential=await provision({relayUrl:origin,enrollmentToken,allowInsecureLoopback});
    ...
    async disable(){++generation;busy=false;config=null;store.set(key,null);const previous=bridge;bridge=null;starting=null;status='disabled';await previous?.close();return info();},
