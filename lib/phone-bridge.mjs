...
export function projectPhoneExport(state){
  ...
  return {format:'cotw-phone-view',version:1,exportedAt:new Date().toISOString(),scope:'Current reserve and retained phone view; private PC diagnostics excluded',selectedReserve:p.selectedReserve,settings:p.settings,zones:p.zones,pins:p.pins,placeLabels:p.placeLabels,encounters:p.encounters,harvests:p.harvests,sessions:p.sessions,route:p.route,routeOptimization:p.routeOptimization,routeSetup:p.routeSetup,...projectZoneData(state)};
}
