...
export async function provisionPhoneDevice({relayUrl,allowInsecureLoopback=false,fetchImpl=fetch,timeoutMs=90000}={}){
  const origin=phoneRelayOrigin(relayUrl,{allowInsecureLoopback});
  const response=await fetchImpl(origin+'/phone/device',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}',redirect:'error',signal:AbortSignal.timeout(timeoutMs)});
...
export function createPhoneBridge({relayUrl,deviceToken,readState,runCommand,onStatus=()=>{},onPairing=()=>{},allowInsecureLoopback=false,WebSocketImpl=globalThis.WebSocket,connectTimeoutMs=90000,reconnectMs=1000}={}){
  ...
  function open(){
    if(!enabled)return;
    const current=++generation,s=new WebSocketImpl(origin.replace(/^http/,'ws')+'/phone/bridge');socket=s;update('connecting');
    const handshake=setTimeout(()=>{if(s.readyState!==1||status!=='connected')s.close();},10000);handshake.unref?.();
    s.addEventListener('open',()=>send(s,{type:'hello',protocol:PHONE_PROTOCOL,deviceToken}));
