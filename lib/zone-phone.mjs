...
export function projectZoneDiscovery(value,enabled,reserve){
  ...
  const statuses=['loading','available','partial','population_unavailable','source_unavailable','reference_unavailable','reference_mismatch'];
...
export function projectZoneData(value){
  ...
  const history=rows((value.zoneHistory||[]).filter(r=>r?.reserve===reserve),r=>({
    ...
    status:['removed','source_unavailable','unrecorded','spoiler_hidden','reference_unavailable'].includes(r.status)?r.status:'source_unavailable',
    ...
    snapshot:r.snapshot&&!['spoiler_hidden','reference_unavailable'].includes(r.status)?own(r.snapshot,'id reserve name x z start end need source species speciesKey groups males females'):null,
