...
export function buildLocationHistory({harvests=[],encounters=[],harvestZones=[],encounterZones=[],zones=[],zoneHistory=[],annotations=[],sessions=[],reserves=[],spoilers=false,sourceStatus='unavailable',zoneSourceStatus=sourceStatus,hiddenZoneIds=[],now=Date.now()}={},input={}){
  ...
  const tracking=sessions.filter(s=>s.endedAt==null&&s.pausedAt==null);
  const selected=query.session==='active'?tracking:[tracking.find(s=>s.id===query.session)].filter(Boolean);
  if(query.session!=='active'&&!selected.length){
    if(sessions.some(s=>s.id===query.session))throw failure('This grind is not tracking. Resume it to see its locations.',409);
  ...
  return {schema:LOCATION_SCHEMA,revision,sourceStatus:source,query,scopeTotal:scope.length,journalEventTotal:scope.length,summary,facets,events:page,offset:query.offset,nextOffset:query.offset+page.length<filtered.length?query.offset+page.length:null,invalid,basis:'Events from running grinds only. Saved harvests plus player-reported shot, death and harvest observations are not a unique kill count. Zone attribution is not exact animal GPS.'};
}
