...
`);
 replace("}catch(e){if(ticket!==requestGeneration||requestedReserve!==reserve||requestedView!==view){pendingRefresh=true;return;}huntSpeciesLoading=false;connectionState(false);$('#connection').textContent='Companion disconnected';", "}catch(e){if(ticket!==requestGeneration||requestedReserve!==reserve||requestedView!==view){pendingRefresh=true;return;}huntSpeciesLoading=false;if(e.cacheDenied||state&&state.selectedReserve!==requestedReserve){state=null;signature='';clearZoneSelection();$('#content').innerHTML='';}connectionState(false);$('#connection').textContent='Companion disconnected';");
 return source;
}
