import {isGreatOneSpecies} from './species-style.js?v=9f326ba1360a70f2';
import {homeBox,validBox,validBounds,insideBounds,scaleBar} from './map-geometry.js?v=9f326ba1360a70f2';
import {TerrainLayer} from './terrain-layer.js?v=9f326ba1360a70f2';
import {routePlan,formatZoneHours} from './route-stops.js?v=9f326ba1360a70f2';
...
export class FieldMap{
 ...
 draw(){if(this.disposed||!this.data||!validBox(this.box))return;this.box=this.viewportBox(this.box);const b=this.box,{reserve,terrain,route}=this.data,rect=this.svg.getBoundingClientRect(),screen=rect.width||850,scale=1/Math.min(screen/b[2],(rect.height||screen)/b[3]);this.svg.setAttribute('viewBox',b.join(' '));for(const [k,v]of Object.entries({x:b[0],y:b[1],width:b[2],height:b[3]}))this.background.setAttribute(k,String(v));
  ...
  const label=(x,z,text,priority=false,color='#fff4db',details=[])=>{
   ...
   if([...labelBoxes,...routeNumberBoxes].some(box=>boxesOverlap(area,box)))return null;
   labelBoxes.push(area);const n=el('text',{x:textX,y:top,'text-anchor':onLeft?'end':'start',fill:color,'font-size':11*scale,'paint-order':'stroke',stroke:'#142017','stroke-width':3*scale,'pointer-events':'none','data-spot-info':details.length?'expanded':'name'});
   for(let index=0;index<lines.length;index++){const line=lines[index],span=el('tspan',{x:textX,y:top+index*14*scale,fill:line.color,'font-size':11*scale},line.text);n.append(span);span.textContent=fitText(span,line.text,room);}
   return n;
  };
