import {isGreatOneSpecies} from './species-style.js?v=263febb61b86541f';
import {homeBox,validBox,validBounds,insideBounds,scaleBar} from './map-geometry.js?v=263febb61b86541f';
import {TerrainLayer} from './terrain-layer.js?v=263febb61b86541f';
import {routePlan,formatZoneHours} from './route-stops.js?v=263febb61b86541f';
...
export class FieldMap{
 ...
 draw(){if(this.disposed||!this.data||!validBox(this.box))return;this.box=this.viewportBox(this.box);const b=this.box,{reserve,terrain,route}=this.data,rect=this.svg.getBoundingClientRect(),screen=rect.width||850,scale=1/Math.min(screen/b[2],(rect.height||screen)/b[3]);this.svg.setAttribute('viewBox',b.join(' '));for(const [k,v]of Object.entries({x:b[0],y:b[1],width:b[2],height:b[3]}))this.background.setAttribute(k,String(v));
  ...
  const same=new Map();for(const z of zones){const key=z.x+':'+z.z;if(!same.has(key))same.set(key,[]);same.get(key).push(z.id);}for(const z of [...zones].sort((a,c)=>Number(c.id===this.data.selectedZone)-Number(a.id===this.data.selectedZone))){const peers=same.get(z.x+':'+z.z),angle=peers.indexOf(z.id)/peers.length*Math.PI*2,dx=peers.length>1?Math.cos(angle)*10*scale:0,dz=peers.length>1?Math.sin(angle)*10*scale:0,x=z.x+dx,y=z.z+dz;
   const name=z.annotation?.name||z.species,g=el('g',{'data-zone':z.id,'data-world-x':z.x,'data-world-z':z.z,tabindex:0,role:'button','aria-label':`${name}, ${z.species}, ${z.need}, X ${Math.round(z.x)}, Z ${Math.round(z.z)}`});if(peers.length>1)g.append(el('line',{x1:z.x,y1:z.z,x2:x,y2:y,stroke:'#fff','stroke-width':scale,opacity:.65}));g.append(el('circle',{cx:x,cy:y,r:7*scale,fill:needColors[z.need]||'#e4ad50',stroke:'#112018','stroke-width':1.5*scale}));g.append(el('title',{},`${name} · ${z.need} · X ${Math.round(z.x)}, Z ${Math.round(z.z)}`));if(b[2]<3500||z.annotation?.name){const t=label(x,y,name,z.id===this.data.selectedZone,isGreatOneSpecies(z.species,z.speciesKey)?'#f0c76d':'#fff4db');if(t)g.append(t);}g.addEventListener('keydown',e=>{if(['Enter',' '].includes(e.key)){e.preventDefault();this.onSelect(z.id);}});out.append(g);
  }
