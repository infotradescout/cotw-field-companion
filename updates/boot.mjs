...
export async function boot(home,{launchDesktop=false,launchBrowser=!launchDesktop,fetcher=fetch,onStarted}={}){
 ...
 try{unlock=acquireLock(home);}catch(e){if(e.code==='UPDATE_LOCKED'){if(launchDesktop)console.log('GrindZone is already running. Bring its desktop window to the front.');else if(launchBrowser&&process.platform==='win32'){const c=spawn('cmd.exe',['/d','/s','/c','start','','http://127.0.0.1:'+context.port],{stdio:'ignore',windowsHide:true});c.on('error',()=>{});}return {status:'already_running'};}throw e;}
 try{
 ...
 return supervisor.supervise({home,trust,context,launchDesktop,launchBrowser,fetcher,onStarted});
}
