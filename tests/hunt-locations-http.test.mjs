...
test('the PC serves location history and its assets without broadening filesystem access',async t=>{
 const {app,save,hashes}=await setup(t);const note=app.observer.command({op:'encounter.create',reserve:19,species:'Whitetail Deer',x:12300,z:8000});
 const response=await fetch(app.url+'/api/locations?kind=shot'),page=await response.json();assert.equal(response.status,200);assert.equal(page.events[0].encounterId,note.id);assert.equal(page.events[0].location.x,12300);
