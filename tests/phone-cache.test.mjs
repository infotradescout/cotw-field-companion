...
import {PhoneSnapshotCache,cacheAuthority,snapshotState,storedView,CACHE_SCHEMA,CACHE_RETENTION_MS} from '../public/phone-cache.js';
...
class MemoryStore{
 ...
 async edit(fn){if(this.fail)throw Error('Storage blocked');const {root,result}=fn(structuredClone(this.root));this.root=structuredClone(root);return result;}
}
function fixture(){
 const store=new MemoryStore();let scope=A,now=NOW,status=200,view=state(),errorScope=null;
 const calls=[];
 const fetchImpl=async url=>{calls.push(url);const boot=url.endsWith('/api/bootstrap');return new Response(JSON.stringify(boot?{selectedReserve:19,token:'csrf-not-stored',phone:{remote:true}}:status===200?view:{error:'PC unavailable'}),{status:boot?200:status,headers:{'content-type':'application/json','x-grindzone-cache-scope':errorScope||scope,'x-grindzone-cache-expires':String(NOW+30*86400000)}});};
 const cache=new PhoneSnapshotCache({store,fetchImpl,now:()=>now});
 return {store,cache,calls,setScope:s=>scope=s,setErrorScope:s=>errorScope=s,setTime:t=>now=t,setStatus:s=>status=s,setView:v=>view=v,async keep(){const v=await cache.get('/api/state?reserve=19');await cache.enable(v);return v;}};
}
...
test('non-phone, mismatched reserve and oversized views are rejected',()=>{
 assert.throws(()=>snapshotState({...state(),phone:{mode:'cached_snapshot'}},19));assert.throws(()=>snapshotState(state(1),19));assert.throws(()=>snapshotState({...state(),harvests:['x'.repeat(3*1024*1024)]},19));
});
