...
export function dashboardData(state,reserve){
 ...
 const grindCount=Number.isFinite(grind?.harvestSummary?.total)?grind.harvestSummary.total:null;
...
function grindMarkup(data){
 ...
 return `<section class="dashboard-grind"><div class="dashboard-section-top"><span class="dashboard-kicker">${current?'CURRENT GRIND':'LAST GRIND'}</span>${current?'<span class="dashboard-live">Tracking</span>':'<span class="dashboard-meta">Finished</span>'}</div><div class="dashboard-grind-main"><div><h2>${esc(grind.name||'My grind')}</h2><span class="dashboard-meta">${current?'Started':'Finished'} ${when(current?grind.startedAt:grind.endedAt)}</span></div><div class="dashboard-grind-count"><strong${grindCount===null?' aria-label="Count unavailable"':''}>${number(grindCount)}</strong><span>harvests</span></div></div><div class="dashboard-grind-foot"><span>All animals · all reserves</span>${current?action('End grind','session-end','','text'):action('New grind','session-start')}</div></section>`;
}
