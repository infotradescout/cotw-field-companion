...
export async function getCatalog(name){
 ...
 const promise=fetch(publicMode?new URL('./catalog/'+name+'.json',import.meta.url):'/api/'+name,{cache:'no-cache'}).then(async r=>{if(!r.ok)throw Error('Reference unavailable. Retry or check the connection.');return r.json();}).catch(e=>{cache.delete(name);throw e;});
