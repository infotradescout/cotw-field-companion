...
class HerdWorkspace extends Base{
 connectedCallback(){
  ...
  this.key=JSON.stringify([this.dataset.source,this.dataset.reserve]);const previous=choices.get(this.key)||{};
  ...
  this.addEventListener('change',e=>{const node=e.target.closest('[data-herd-filter]');if(!node)return;e.stopPropagation();this.filters[node.dataset.herdFilter]=node.value;this.offset=0;this.remember();this.load();},{signal:this.events.signal});
 ...
 async click(e){const b=e.target.closest('[data-herd-map],[data-herd-retry],[data-herd-page]');if(!b)return;e.stopPropagation();
 ...
 render(){
  ...
  const content=this.error?`<p role="status" class="gz-herd-notice">${esc(this.error)}</p>`:d?.status==='spoilers_off'?'<p class="gz-herd-notice">Herd membership and population trophies are hidden. Enable population spoilers in Settings to view your herds and assigned zones.</p>':d?.status==='unavailable'?'<p class="gz-herd-notice">No matching readable herd snapshot is available yet. Open the updated PC companion and let the game save.</p>':d?`${status?`<p class="gz-herd-notice" role="status">${status}</p>`:''}<div class="gz-herd-totals"><div><strong>${num(s.herds)}</strong><span>Herds</span></div><div><strong>${num(s.animals)}</strong><span>Saved animals</span></div><div><strong>${num(s.diamonds)}</strong><span>Diamond potential</span></div><div><strong>${num(s.greatOnes)}</strong><span>Great Ones · saved flags</span></div></div><p class="gz-herd-caption">${s.greatOneCandidates?num(s.greatOneCandidates)+' additional Great One candidates. ':''}${s.unclassifiedHerds?num(s.unclassifiedHerds)+' herds lack a complete trophy reference. ':''}Last saved ${esc(when(d.savedAt))}</p><div class="gz-herd-filters">${select('species','Species',[['all','All species'],...(d.facets.species||[]).map(r=>[r.key,r.name+(r.femaleDiamondCapable===true?' ♀':'')])])}${select('zone','Zone → herds',[['all','All assigned zones'],...(d.facets.zones||[]).map(z=>[z.id,(z.name||'Zone')+' · '+z.need+' · '+z.herds.map(h=>h.label).join(', ')])])}${select('trophy','Trophy potential',[['all','All herds'],['diamond','Diamond potential'],['great_one','Great Ones / candidates'],['female_diamond','Female Diamond-capable species']])}</div>${this.selected?'<div class="gz-herd-map"><svg role="application" aria-label="Selected herd’s assigned need zones"></svg><p data-herd-map-status role="status">Saved zone assignments, not live movement.</p></div>':''}<div class="gz-herd-cards">${rows.length?herdCards(rows):'<p>No herds match these filters.</p>'}</div><div class="gz-herd-pages"><button type="button" data-herd-page="${Math.max(0,this.offset-25)}" ${this.offset?'':'disabled'}>Previous</button><span>${num(this.offset+(rows.length?1:0))}–${num(this.offset+rows.length)} of ${num(s.herds)}</span><button type="button" data-herd-page="${d.nextOffset??0}" ${d.nextOffset===null?'disabled':''}>Next</button></div><details><summary>How IDs and trophies are established</summary><p>${esc(d.identityNotice)}</p><p>${esc(d.trophyNotice)}</p><p>${esc(d.movementNotice)}</p><p>♀ identifies species whose females can reach Diamond; it does not label an individual’s sex or medal.</p></details>`:'<p role="status">Loading herd identities and zones…</p>';
  this.innerHTML=`<link rel="stylesheet" href="${esc(new URL('./herd-view.css',import.meta.url).href)}"><section class="gz-herd-workspace"><header class="gz-herd-heading"><div><span>YOUR SAVED POPULATION</span><h2>Herds &amp; their zones</h2></div><button type="button" data-herd-retry>Refresh</button></header>${this.lifetime}${content}</section>`;
